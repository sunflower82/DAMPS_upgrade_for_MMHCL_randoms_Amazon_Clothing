"""scripts/run_p8_1_head_preserve.py -- P8.1 Head-preservation on k2_e256.
=============================================================================

Rationale
---------
k2_e256 (P7.1 K-block winner) achieved R@20=0.10234 / NDCG@20=0.04686 at
seed 23946202 but the WandB CSV shows Head@20 peaked early and decayed:

    Head@20:   ep39 = 0.15299  ->  ep84 = 0.14339   (Delta = -0.00960 = -6.3%)
    Mid@20:    ep84 = 0.05167  (still climbing, peak at last epoch)
    Tail@20:   ep83 = 0.02309  (still climbing)
    NDCG@20:   ep60 = 0.04710  (peak, then mild decay)
    val R@20:  ep54 = 0.10179  (peak, restored by early stopping)

The Head-decay indicates the model over-fits popular items after ~ep 50
while Mid/Tail are still improving.  P8.1 tests four different
Head-preserving levers on top of the k2_e256 config to see which recovers
Head loss without regressing Mid/Tail.

Grid design (4 variants x 1 seed x 100 (or 150) epoch  ~=  2 - 3 h)
------------------------------------------------------------------

Anchor:  embed_size=256, UI_layers=3, weight_size=[64,64,64] (from k2_e256).

    p8_e256_logq055     logq_scale = 0.55     (softer pop de-bias -- b2_logq055)
    p8_e256_regs2x      regs = 9.6e-4         (2x current L2)
    p8_e256_regs4x      regs = 1.92e-3        (4x current L2)
    p8_e256_lr_half     lr = 1.25e-4 + epoch=150 + patience=45
                             (smoother trajectory -- delay Head decay)

Usage
-----
::

    python scripts/run_p8_1_head_preserve.py \\
        --seeds 23946202 \\
        --epoch 100 \\
        --output ./results/p8_1_head_preserve_clothing.json \\
        --dry_run 0

Note
----
p8_e256_lr_half uses epoch=150 + patience=45 internally to compensate for
the slower learning rate.  If wall time is tight, add `--only_tags` to
drop it and start with the three fast variants.
"""
...
BASELINE_CLI = {
    ...
    "wandb_group": "p8_1_head_preserve",
}


def build_grid(base_cache: str) -> list[dict]:
    grid = [
        # p8_e256_logq055 -- softer pop de-bias, borrows §9.28 b2_logq055 lever
        {"tag": "p8_e256_logq055", "block": "P8_1_logq",
         "overrides": {"logq_scale": 0.55,
                       "tamer_interest_cache": base_cache}},
        # p8_e256_regs2x -- 2x current L2
        {"tag": "p8_e256_regs2x", "block": "P8_1_regs",
         "overrides": {"regs": 9.6e-4,
                       "tamer_interest_cache": base_cache}},
        # p8_e256_regs4x -- 4x current L2 (decisive overfit test)
        {"tag": "p8_e256_regs4x", "block": "P8_1_regs",
         "overrides": {"regs": 1.92e-3,
                       "tamer_interest_cache": base_cache}},
        # p8_e256_lr_half -- lr = 1.25e-4, epoch=150, patience=45
        {"tag": "p8_e256_lr_half", "block": "P8_1_lr",
         "overrides": {"lr": 1.25e-4,
                       "epoch": 150,
...
def build_cli(python_exe: str, main_py: Path, variant: dict,
    ...
    cfg["seed"] = seed
    cfg["epoch"] = epoch
    cfg.update(variant["overrides"])   # may override epoch for lr_half
    cfg["wandb_run_name"] = f"{variant['tag']}_seed{seed}"
    cfg["wandb_tags"] = ",".join([
        "p8", "p8_1_head_preserve", variant["block"], variant["tag"],
...
def main() -> None:
    ...
    ap.add_argument("--output", type=Path,
                    default=Path("./results/p8_1_head_preserve_clothing.json"))
    ap.add_argument("--log_dir", type=Path,
                    default=Path("./results/_p8_1_head_preserve_logs"))
    ...
    for variant in grid:
        for seed in args.seeds:
            ...
            row = {
                ...
                "block": variant["block"],
                "seed": seed,
                "epoch_cap": variant["overrides"].get("epoch", args.epoch),
                "wall_min": wall / 60.0,
                "exit": exit_code,
                **variant["overrides"],
                **parsed,
    ...
    k2_ref   = {"r20": 0.10234, "ndcg20": 0.04686,
                "head": 0.15229, "mid": 0.04147, "tail": 0.01762}
    ...
    for tag, rs in per_tag.items():
        ...
        ranked.append({
            ...
            "delta_vs_k2_head":  (h - k2_ref["head"])    if h else None,
        })
    ...
    with args.output.open("w", encoding="utf-8") as fh:
        json.dump({
            ...
            "k2_e256_anchor": k2_ref,
            "runs_completed": len(rows), "total_runs": total_runs,
        }, fh, indent=2)

    print("\n=== P8.1 Head-preserve (ranked by R@20) ===")
    print(f"{'tag':<20} {'R@20':>8} {'dk2':>8} {'NDCG':>8} "
          f"{'dk2N':>8} {'Head':>8} {'dk2H':>8} {'Mid':>8} {'Tail':>8}")
    for r in ranked:
        ...
        dk2h = r["delta_vs_k2_head"] or 0.0
        m = r["mid_mean"] or float("nan")
        t = r["tail_mean"] or float("nan")
        print(f"{r['tag']:<20} {r20:>8.5f} {dk2:>+8.5f} "
              f"{nd:>8.5f} {dk2n:>+8.5f} "
              f"{h:>8.5f} {dk2h:>+8.5f} "
              f"{m:>8.5f} {t:>8.5f}")
    ...
    if d > 0.0005:
        print("  Signal above k2_e256. Replicate 2 seeds before adopting.")
    elif d > -0.0005:
        print("  Within noise band of k2_e256. Head-preservation not needed.")
    else:
        print("  Regression vs k2_e256. Keep k2_e256 config.")
