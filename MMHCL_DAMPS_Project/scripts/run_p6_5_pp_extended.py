"""scripts/run_p6_5_pp_extended.py -- P6.5'' extended-training sanity check.
============================================================================

Ships Priority 6.5'' (P6.5-double-prime): a *pure CLI* extended-training
sanity check on top of the P6.4 TAMER winner (``alpha_interest = 0.50``).

Rationale
---------
P6.5' (commit ``df76a25``) falsified the popularity-inverse hypothesis:
tail R@20 crashed −54.3% between ep 4 and ep 19 when pop-inverse cut the
(tail, head) bridge edges, and the whole grid ended below the P6.4 baseline.

Before spending code cycles on P6.7 (tail-aware BPR sampling) or P6.8
(modality-fallback tail bridges), we need to rule out the *cheapest*
alternative hypothesis:

    "P6.4 was simply undertrained at epoch=100."

Diagnostic signals from the P6.4 CSVs:
  * Overall R@20 still micro-oscillating between 0.0923 and 0.0925 at ep 92-99
  * Head R@20 already declining from its ep-44 peak (-3.4pp)
  * Mid R@20 monotonically rising, slope +0.20%/epoch at ep 80-99
  * Tail R@20 monotonically rising, slope +0.74%/epoch at ep 80-99

If we uncap training at epoch=200 with patience=50 and val_recall@20 does
NOT keep climbing, the P6.4 plateau is real and we must attack the loss
side (P6.7). If R@20 climbs meaningfully (>+0.5pp overall or Tail crosses
0.018), P6.4 was undertrained and every downstream diagnostic needs
re-baselining.

Config (bit-for-bit P6.4 winner + extended budget)
--------------------------------------------------
  * ``alpha_interest = 0.50``     (P6.4 winner)
  * ``pop_inverse_eta = 0.0``     (P6.4 bit-for-bit)
  * ``--epoch 200``               (2x P6.4)
  * ``--early_stopping_patience 50`` (up from 30, so ~1/4 of budget)
  * ``--early_stopping_monitor val_recall@20`` (NOT bucket_geo; we are
    explicitly asking whether the standard PACER monitor still improves)
  * ``--early_stopping_min_delta 1e-4`` (unchanged)
  * ``--eval_every 5`` for ep <= 60, then per-epoch for ep 60-200 via
    ``--eval_last_epochs 140`` so the tail of the curve is dense.

Seeds
-----
Locked pair ``(23946202, 1557638902)`` (first two locked seeds -- match
P6.1 / P6.2 / P6.3 / P6.4 / P6.5' for direct comparison).

Wall-clock estimate
-------------------
~24-26 min/seed for 100 epochs on RTX 5090 -> ~50-55 min/seed at 200 epochs
if patience does NOT bite. Grid total: ~110 min wall.

Cache reuse
-----------
Consumes the SAME ``results/interest_tree_clothing.npz`` cache used by
P6.4 / P6.5'. No cache surgery.

Usage (from MMHCL_DAMPS_Project/)::

    # Full grid (2 seeds, extended):
    python scripts/run_p6_5_pp_extended.py

    # Dry run:
    python scripts/run_p6_5_pp_extended.py --dry_run 1

    # Single seed:
    python scripts/run_p6_5_pp_extended.py --seeds 23946202
"""
...
# --------------------------------------------------------------------------- #
#  Locked P6.4 winner trunk (identical to run_p6_5_bucket_aware.py minus
#  the P6.5' overrides). Any drift here breaks the comparability with P6.4.
# --------------------------------------------------------------------------- #
DATASET = "Clothing"
EPOCH_DEFAULT = 200          # <- P6.5'' change (was 100 in P6.4)
PATIENCE_DEFAULT = 50        # <- P6.5'' change (was 30 in P6.4)
EVAL_LAST_EPOCHS = 140       # <- P6.5'' change: dense per-epoch eval for ep 60-200
BATCH_SIZE = 1024
LR = 2.50995e-4
REGS = 1.20e-04
EMBED_SIZE = 128
...
# P6.5'' explicitly turns pop-inverse OFF (bit-for-bit P6.4 compatibility).
POP_INVERSE_ETA_LOCK = 0.0

SEEDS_DEFAULT: tuple[int, ...] = (23946202, 1557638902)

OUT_JSON = "results/p6_5_pp_extended_clothing.json"
CACHE_PATH = "results/interest_tree_clothing.npz"

# P6.4 winner references (mean of 2 locked seeds, capped at ep=100).
...
class GridConfig:
    tag: str  # single cell, but keep the shape for JSON-compat with P6.5'


def build_configs() -> list[GridConfig]:
    return [GridConfig(tag="p6_5_pp_extended")]
...
def _base_flags(*, cache_path: Path, epoch: int, patience: int) -> list[str]:
    """P6.4 winner base flags + extended budget.

    IMPORTANT: keep this list synchronised with ``run_p6_5_bucket_aware.py``
    minus the P6.5' overrides (``pop_inverse_eta`` and the bucket_geo monitor).
    Any drift breaks direct comparability with the earlier grids.
    """
    return [
        ...
        # P6.5'' key change: patience = 50, monitor = val_recall@20.
        ...
        # ---- P6.5'' explicitly disables pop-inverse (P6.4 bit-for-bit)
        "--pop_inverse_eta", str(POP_INVERSE_ETA_LOCK),
        # ---- WandB ----------------------------------------------------
        "--use_wandb", "1",
        "--wandb_project", "damps-mmhcl-clothing",
        "--wandb_entity", "baitapck51cc-uet",
        "--wandb_group", "p6_5_pp_extended",
        "--wandb_tags",
        "p6,p6_5_pp,extended_training,tamer,alpha050,epoch200,patience50,"
        "nrdmc_lite,sanity_check",
    ]
...
def _run_one(*, python_exe: str, damps_dir: Path, cfg: GridConfig,
    ...
    run_name = f"p6_5_pp_extended_seed{seed}"
    cmd = [
        python_exe, "main_tercile.py",
        *_base_flags(cache_path=cache_path, epoch=epoch, patience=patience),
        "--seed", str(seed),
        "--wandb_run_name", run_name,
    ]
    print(
        f"\n{'=' * 74}\n"
        f"[P6.5''] cfg={cfg.tag}  alpha_interest={ALPHA_INTEREST_LOCK}  "
        f"pop_inverse_eta={POP_INVERSE_ETA_LOCK} (OFF)  "
...
def main():
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Alias for --out (used by notebook cell 9.26).",
    )
    p.add_argument(
        "--cache",
        type=str,
    ...
    # Aggregate + rank vs the P6.4 100-epoch reference so the notebook cell
    # can print a one-line verdict (undertrained vs plateau).
    ...
    for tag, s in agg.items():
        ...
        # P6.5'' verdict logic (printed only, not persisted -- interpretation
        # belongs in the paper/notebook, not in machine-consumed JSON):
        #   climbed  = R@20 mean > P6.4 baseline + 0.5pp
        #   plateau  = |R@20 mean - P6.4 baseline| < 0.5pp
        #   worse    = R@20 mean < P6.4 baseline - 0.5pp
        dr = r20["mean"] - P64_MEAN_R20
        ranked.append({
            "tag": tag,
            "alpha_interest": ALPHA_INTEREST_LOCK,
            "pop_inverse_eta": POP_INVERSE_ETA_LOCK,
            ...
            "delta_vs_p6_4_r20": dr,
            "delta_vs_p6_4_ndcg": n20["mean"] - P64_MEAN_NDCG20,
            "delta_vs_p6_4_tail": tail["mean"] - P64_MEAN_R20_TAIL,
            "n_seeds": r20["n"],
        })
    ...
    print("\n=== P6.5'' summary (vs P6.4 100-epoch baseline) ===")
    print(
        f"{'tag':22s}  R@20     dR20      NDCG      dNDCG    "
        f"Tail      dTail    best_ep"
    )
    for r in ranked:
        print(
            f"{r['tag']:22s}  {r['recall20_mean']:.5f}  "
            f"{r['delta_vs_p6_4_r20']:+.5f}  "
            f"{r['ndcg20_mean']:.5f}  {r['delta_vs_p6_4_ndcg']:+.5f}  "
            f"{r['tail_mean']:.5f}  {r['delta_vs_p6_4_tail']:+.5f}  "
